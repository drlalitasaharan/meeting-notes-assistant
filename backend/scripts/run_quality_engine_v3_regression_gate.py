#!/usr/bin/env python3
"""Run the Quality Engine v3 regression gate on known fixture meetings.

This script is local/QA-only. It does not enable Quality Engine v3 rollout and
...
from app.services.quality_engine_v3_regression_gate import (  # noqa: E402
...
DEFAULT_OUTPUT = Path("backend/reports/qev3_regression_gate/latest.json")
DEFAULT_MARKDOWN_OUTPUT = Path("backend/reports/qev3_regression_gate/latest.md")


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run Quality Engine v3 regression quality gate on fixture meetings."
    )
    parser.add_argument(
        "--fixture-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.70,
        help="Minimum v3 case score required to pass.",
    )
    parser.add_argument(
        "--allow-fail",
        action="store_true",
...
def build_report(
    *,
    ...
    gate_report = evaluate_quality_engine_v3_regression_gate(
        gate_cases,
        config=QualityEngineV3RegressionGateConfig(min_v3_case_score=min_score),
    )
    case_metadata = {
        case["case_id"]: {
...
def render_markdown_report(report: dict[str, Any]) -> str:
    lines = [
        "# Quality Engine v3 Regression Gate",
        "",
        "This report is generated from fixture transcripts only. It does not enable "
        "Quality Engine v3 rollout or change production note behavior.",
        "",
        f"- Fixture directory: `{report['fixture_dir']}`",
        f"- Cases: {', '.join(report['case_ids'])}",
        f"- Minimum v3 score: {report['min_score']}",
        f"- Result: {'PASS' if report['passed'] else 'FAIL'}",
        "",
        "| Case | v1 score | v3 score | Result | Failed checks |",
        "| --- | ---: | ---: | --- | --- |",
    ]
    ...
    for result in report["results"]:
        ...
        lines.append(
            "| {case_id} | {v1_score} | {v3_score} | {status} | {failed} |".format(
                case_id=result["case_id"],
                v1_score=result["v1_score"],
                v3_score=result["v3_score"],
